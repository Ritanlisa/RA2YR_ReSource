...
- **2026-09-05 (终)**: 导入闭环 + 终态清点 — CSP 全通道落盘 IDB
  - **mass_type 重跑**: 最小 struct 放行 → **+961 个 this 签名**（累计 ~9,840 应用, retruth 真值修正累计 612）; 1 失败（RTtypeid CRT 折叠, 非函数起始地址）
  - **t14_apply_sigs**: 参数/返回通道 **1,419 处编辑落盘**（return+param）
  - **全局变量通道正式化**（`.omo/t14_globals_apply.py`→后续应入库）: 类类型具名全局 1,925 中 **449 应用**（+12 幂等）; 1,463"未解析"实为 _RET 伪节点漏滤（非全局）
  - **struct 终态**: til 共 1,474 个（1,252 全布局 + 20 成员建 + 202 最小 vfptr）; 门 661 类覆盖 119,106 变量, 缺口 13（5 枚举+1 数组伪型+7 零观测）
  - **CSP 终态**: type_map 732,742 / 类类型 138,884 / TOP=0 / ORPHAN 20.6%; this 10,767/12,938; SSA 103,821; 成员 8,203; 栈槽 10,193; 参数 3,992; 返回 127/8,432; 局部变量 IDB 累计 **~19,695**
