...
def generate(markers, functions, fn_map, none_markers=None):
    ...
    w('}')
    w('')
    # In-memory section buffers for direct-slot writing
    w('#define SEC_BUF_SZ 65536')
    w('static char sec_cap[SEC_BUF_SZ]={0}; static int sec_cap_len=0;')
    w('static char sec_diff[SEC_BUF_SZ]={0}; static int sec_diff_len=0;')
    w('static char sec_same[SEC_BUF_SZ]={0}; static int sec_same_len=0;')
    ...
    w('  if(sec_diff_len>0){{DWORD n;WriteFile(h,sec_diff,sec_diff_len,&n,0);}}')
    w('  Wr("\\r\\n================ Captures ================\\r\\n");')
    w('  if(sec_cap_len>0){{DWORD n;WriteFile(h,sec_cap,sec_cap_len,&n,0);}}')
    w('  Wr("\\r\\n============= Same Compares ==============\\r\\n");')
    w('  if(sec_same_len>0){{DWORD n;WriteFile(h,sec_same,sec_same_len,&n,0);}}')
    w('  Wr("\\r\\n============== None Calls ================\\r\\n");')
    ...
    w('  if(h!=INVALID_HANDLE_VALUE){{')
    w('    DWORD dn;')
    w('    if(n==1) WriteFile(h,hdr_buf,lstrlenA(hdr_buf),&dn,0);')
    w('    WriteFile(h,ebuf,(DWORD)elen,&dn,0);')
    w('  }}')
    w('  // Section buffer: header only on first call, body always')
    w('  char* s; int* pl;')
    w('  if(tag==\'C\'){s=sec_cap;pl=&sec_cap_len;}')
    w('  else if(tag==\'I\'&&has_diff[i]){s=sec_diff;pl=&sec_diff_len;}')
    w('  else{s=sec_same;pl=&sec_same_len;}')
    w('  if(n==1) SecAppend(s,pl,hdr_buf);')
    w('  SecAppend(s,pl,ebuf);')
    w('  rebuild_none();')
    w('  // Smart flush: diff entries immediate, first 100 immediate, else batched')
