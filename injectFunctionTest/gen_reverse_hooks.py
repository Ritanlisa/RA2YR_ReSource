...
def generate(markers, functions, fn_map):
    ...
    w('}')
    # Log
    w('static void Log(DWORD a,int i,DWORD re,DWORD orig,DWORD ret,bool capture){')
    w('  if(!f){f=fopen("comparisonResult.log","w");if(!f)return;}')
    w('  int n=++ctr[i];')
    w('  if(n==1){')
    w('    long sz=ftell(f);fprintf(f,"%s[%s-0x%08X]\\n",sz>0?"\\n":"",nm[i]?nm[i]:"?",a);')
    w('  }')
    ...
    # Write section headers and No Call entries
    w('  fprintf(f,"\\n========== Capture Results ==========\\n");')
    w(f'  for(int i=0;i<{nmk};++i){{')
    w('    if(is_cap[i] && ctr[i]==0){')
    w('      fprintf(f,"\\n[%s-0x%08X]\\nNo Call\\n",nm[i]?nm[i]:"?",addr_tbl[i]);')
    w('    }')
    w('  }')
    w('  fprintf(f,"\\n========== Different Compares ==========\\n");')
    w(f'  for(int i=0;i<{nmk};++i){{')
    w('    if(!is_cap[i] && has_diff[i]){')
    w('      fprintf(f,"\\n[%s-0x%08X]\\n%d call(s), %d difference(s)\\n",nm[i]?nm[i]:"?",addr_tbl[i],ctr[i],ctr[i]);')
    w('    }')
    w('  }')
    w('  fprintf(f,"\\n============ Same Compares ============\\n");')
    w(f'  for(int i=0;i<{nmk};++i){{')
    w('    if(!is_cap[i] && !has_diff[i] && ctr[i]==0){')
    w('      fprintf(f,"\\n[%s-0x%08X]\\nNo Call\\n",nm[i]?nm[i]:"?",addr_tbl[i]);')
    w('    }')
    w('  }')
    ...
    for m in markers:
        ...
        if mode == "Capture":
            w('  V.re=0;')
            w(f'  Log(0x{ah}, idx, V.re, V.re, R->Stack<DWORD>(0), true);')
    ...
    w('  Log(addr,idx,V.re,orig,ret,false);')
    w('}')
    return '\n'.join(out)
