...
def generate(markers, functions, fn_map, none_markers=None):
    ...
    w('#include <string>')
    w('#include <iomanip>')
    w('#include <gamemd/core/math.hpp>')
    w('using gamemd::Point2D; using gamemd::RectangleStruct;')
    w('inline std::ostream& operator<<(std::ostream& os, const Point2D& p){ return os<<\"<Point2D,X=\"<<std::dec<<p.X<<\",Y=\"<<p.Y<<\">\"; }')
    w('inline std::ostream& operator<<(std::ostream& os, const RectangleStruct& r){ return os<<\"<RectangleStruct,X=\"<<std::dec<<r.X<<\",Y=\"<<r.Y<<\",W=\"<<r.Width<<\",H=\"<<r.Height<<\">\"; }')
    w('')
    ...
    w('static void Hex8(std::ostream& os, DWORD v){ char f=os.fill(\'0\'); os<<\"0x\"<<std::hex<<std::uppercase<<std::setw(8)<<v<<std::dec; os.fill(f); }')
    w('')
    # Format: type-aware value output to ostringstream
    w('static void Fmt(std::ostream& os, const char* ty, DWORD v){')
    w('  if(!ty)ty="";')
    w('  if(strstr(ty,"Point2D")){os<<*(const Point2D*)v<<"(";Hex8(os,v);os<<")";return;}')
    w('  if(strstr(ty,"RectangleStruct")){os<<*(const RectangleStruct*)v<<"(";Hex8(os,v);os<<")";return;}')
    w('  if(strstr(ty,"bool")||strstr(ty,"Bool")) os<<(v?"true":"false")<<"("; else os<<std::dec<<v<<"(";')
    w('  Hex8(os,v); os<<")";')
    w('}')
