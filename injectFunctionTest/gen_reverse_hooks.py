...
def generate(markers, functions, fn_map, none_markers=None):
    ...
    w('  }')
    w('')
    w('  // Section 4: None Calls — list None-mode markers whose active counterpart not called')
    w('  fprintf(f,"\\n============== None Calls ================\\n");')
    # For each None-mode marker, check if any active marker with same addr has ctr>0
    w('  int any_none=0;')
    for i,m in enumerate(none_markers):
        sig = m.get('full_sig', m['fn_name']).replace('\\','\\\\').replace('"','\\"').replace('\n',' ')
        ...
        w(f'    }}')
        w(f'    if(!called){{ fprintf(f,"[{sig}-{addr_hex}]\\n"); any_none=1; }} }}')
    w('  if(!any_none) fprintf(f,"(all None-mode functions were called)\\n");')

    # Also list active markers with ctr==0 that aren't None-mode equivalents
    w(f'  for(int j=0;j<{nmk};j++){{')
    w('    if(ctr[j]==0){')
    # Check not already listed as None-mode
    w('      int already=0;')
    w(f'      for(int k=0;k<{len(none_markers) if none_markers else 1};k++){{')
    w('        unsigned na=0;')
    for i,m in enumerate(none_markers):
        w(f'        if(k=={i}) na={int(m["addr"],16)};')
    w('        if(addr_tbl[j]==na) already=1;')
    w('      }')
    w('      if(!already){')
    ...
    w('  }')
    w('  fflush(f);')
    w('  static int entry_count=0;')
    w('  if(++entry_count % 10 == 0) rewrite_sections();')
    w('}')
