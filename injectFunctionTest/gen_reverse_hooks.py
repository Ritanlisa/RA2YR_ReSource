...
def generate(markers, functions, fn_map, none_markers=None):
    ...
    w('inline std::ostream& operator<<(std::ostream& os, const Point2D& p){ return os<<\"<Point2D,X=\"<<p.X<<\",Y=\"<<p.Y<<\">\"; }')
    w('inline std::ostream& operator<<(std::ostream& os, const RectangleStruct& r){ return os<<\"<RectangleStruct,X=\"<<r.X<<\",Y=\"<<r.Y<<\",W=\"<<r.Width<<\",H=\"<<r.Height<<\">\"; }')
    w('')
