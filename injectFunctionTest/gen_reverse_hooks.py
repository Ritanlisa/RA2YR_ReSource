...
def generate(markers, functions, fn_map, none_markers=None):
    ...
    w('  WrF("[%s-0x%08X]\\r\\n",sig[i]&&*sig[i]?sig[i]:nm[i]?nm[i]:"?",addr);')
    w('  WrF("Call %d: %s()<-0x%08X\\r\\n",n,cn?cn:"?",ret-5);')
    w('  FI(i);')
    w('  if(tag==\'C\'){ WrF("  Return: 0x%08X\\r\\n",orig); }')
    w('  else if(re==orig){ WrF("  Return: hook=original=0x%08X\\r\\n",orig); }')
    w('  else{ has_diff[i]=true; WrF("  Return: hook=0x%08X != original=0x%08X\\r\\n",re,orig); }')
    w('  char* s; int* pl;')
