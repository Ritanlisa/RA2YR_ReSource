#include "dynamicvectorclass_animtype.hpp"

uint32_t * DynamicVectorClass_AnimType::Add(uint32_t *Block, char a2)
{
    void *v3; // eax
    v3 = (void *)*(Block + 1);
...
char  DynamicVectorClass_AnimType::_vt02(void *this, int a2, uint32_t *a3)
{
    uint32_t *v4; // edi
    int v6; // ecx
    int i; // eax
...
int  DynamicVectorClass_AnimType::_vt04(int this, uint32_t *a2)
{
    int result; // eax
    int v3; // edx
    uint32_t *i; // ecx
...
// 0x00525230
unsigned int  DynamicVectorClass_AnimType::Remove(int this, int a2)
{
    if ( *(uint8_t *)(this + 12) )
