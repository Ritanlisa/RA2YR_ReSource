#include "vectorclass_rc_ptr_wdtterritory.hpp"

uint32_t * VectorClass_rc_ptr_WDTTerritory::_vt06(uint32_t *this, uint32_t *a2, int a3)
{
    int v3; // edi
    v3 = *(this + 1) + 8 * a3;
    ComPtr::Constructor(a2);
    *a2 = &rc_ptr<WDTTerritory>::`vftable';
    ComPtr::Set(a2, *(uint32_t *)(v3 + 4));
    return a2;
}

char  VectorClass_rc_ptr_WDTTerritory::_vt01(int *this, int a2)
{
    int v2; // esi
    int v3; // edx
    uint32_t *v4; // ecx
...
char  VectorClass_rc_ptr_WDTTerritory::_vt02(uint8_t *this, int a2, uint32_t *a3)
{
    int v3; // ebp
    uint32_t *v5; // eax
    uint32_t *v6; // esi
    ...
    *v6 = &rc_ptr<WDTTerritory>::`vftable';
    v6 += 2;
    --v9;
    while ( v9 );
    v6 = a3 + 1;
...
int  VectorClass_rc_ptr_WDTTerritory::_vt03(int this)
{
...
int  VectorClass_rc_ptr_WDTTerritory::_vt04(int this, int a2)
{
    int result; // eax
    int v3; // edx
    uint32_t *i; // ecx
...
unsigned int  VectorClass_rc_ptr_WDTTerritory::_vt05(int this, int a2)
{
    if ( *(uint8_t *)(this + 12) )
...
uint32_t * VectorClass_rc_ptr_WDTTerritory::_vt00(uint32_t *Block, char a2)
{
