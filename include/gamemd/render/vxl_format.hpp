...
// A VXL file contains:
//   1. Header (16 bytes)
//   2. Section table (num_sections × 28 bytes in file, stored as 12 bytes)
//   3. Body data (raw voxel grid)
//   4. Frame table (num_frames × 92 bytes in file, stored as 164 bytes)
...
struct VXLFrame
{
    ...
    // Storage order: row-major 3×4 matrix (position + rotation + scale)
    ...
    float    bounds[12];       // 4 × 3D corner points of bounding box
...
// HVA Section Frame — 48 bytes (12 floats = 3×4 matrix)
// Used for turret/barrel/extra section animation
struct HVASectionMatrix
{
    float matrix[12];       // 3×4 affine transform per section per frame
};
...
struct VoxelBody
{
    ...
    VXLSection* sections;   // Array of section descriptors (num_sections × 12B)
    VXLFrame*   frames;     // Array of frame data (num_frames × 164B)
...
struct VoxelAnim
{
    ...
    HVASectionMatrix*   matrices;      // (num_frames × num_sections) × 48B entries
};
